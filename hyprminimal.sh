...
do
  read -p "Hi $u, do you want to install Hyprland dependecies? (Y/n) " resp      # user response
    ...
    if [[ $resp == "N" ]] || [[ $resp == "n" ]]; then 
      ...
      paru -Syu hyprland xdg-desktop-portal-hyprland xdg-desktop-portal-gtk polkit-kde-agent hyprpaper hyprlock hypridle hyprshot wl-clipboard rofi-wayland foot librewolf-bin nwg-look thunar thunar-archive-plugin gvfs gvfs-gphoto2 gvfs-afc xarchiver imagemagick ristretto brightnessctl unzip unrar p7zip htop alsa-utils dosfstools papirus-icon-theme gnome-themes-extra ttf-jetbrains-mono-nerd adobe-source-han-sans-jp-fonts

      echo -e "${GREEN}AUR packages installed.${RC}"
      
      cd hyprminimal/ && cp -r * ~/.config/ 
